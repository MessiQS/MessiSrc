...
export default class Find extends Component {
  ...
  static navigationOptions = ({ navigation, screenProps }) => ({
    ...
    headerLeft: (
      <View style={header.header}>
        <Text style={header.text}>刷题统计</Text>
      </View>
    ),
    headerRight: (
      <View style={header.header}>
  ...
  prepareWeChat() {
    ...
    WeChat.isWXAppInstalled()
      .then((isInstalled) => {
        that.isInstalledWeChat = isInstalled
      });
  }

  showMore() {
    console.log("this.installedWeChat", this.isInstalledWeChat)
    this.setState({
    ...
    setTimeout(() => {
      ...
      .then((isInstalled) => {
        ...
        if (isInstalled == false) {
          that.params.items.splice(0, 0, { text: "账号信息", type: "Normal", handler: () => handleAccount() })
        }

        that.props.navigation.setParams({
          route: that.showMore.bind(that)
        })
      });
    }, 1)
  }
